...
def generate_app(get_processed_inputs, inpaint):
    ...
    def run_sam():
        ...
        try:
            ...
            res_mask = np.array(Image.fromarray(
                mask).resize((IMG_SIZE, IMG_SIZE)))
    ...
    def run(prompt, negative_prompt, cfg, seed, invert):
        ...
        try:
            inpainted = inpaint(input_image, amask, prompt,
    ...
    with gr.Blocks() as demo:
        ...
        # Events
        display_img.select(get_points, inputs=[display_img], outputs=[
                           sam_mask, display_img])
        display_img.clear(reset_points)
        display_img.change(preprocess, inputs=[
                           display_img], outputs=[display_img])
